class Storage {
  constructor() {
    ...
    this.indexedStorageEnabled = false;
  }
  ...
  async checkIndexedDbAvailable() {
    ...
    if (window.indexedDB) {
      const dbPromise = new Promise((resolve, reject) => {
        const dbReq = window.indexedDB.open('__test__', 1);
      ...
      try {
        await dbPromise;
        this.indexedStorageEnabled = true;
        console.info("Indexed storage is available.");
      } catch (e) {
        console.info("Indexed storage is not available: " + e);
      }
    }
  }
}
